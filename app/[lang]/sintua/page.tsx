...
export async function generateMetadata({
  ...
  return pageMetadata("/sintua", locale, getDictionary(locale).meta.sintua);
}

export default async function SintuaPage({
  params,
}: {
  params: Promise<{ lang: string }>;
}) {
  const { lang } = await params;
  const locale: Locale = isLocale(lang) ? lang : "id";
  const t = getDictionary(locale).sintua;
  return (
    <>
      <Header />
      <main className="flex-1">
        ...
        <section id="sintua" className="mx-auto max-w-4xl px-5 py-20 md:py-24">
          ...
          <p className="mt-5 text-sm text-black/55">{t.footnote}</p>
        </section>
      </main>
      <Footer />
    </>
